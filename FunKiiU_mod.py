...
def download_file(url, outfname, retry_count=3, ignore_404=False, expected_size=None, resultsq=None, titleid=None, threadid=None, chunk_size=0x4096):
    for _ in retry(retry_count):
        ...
        try:
            ...
            if expected_size != diskFilesize:
                if resultsq:
                    resultsq.put(('status', threadid, 'Downloading {} for title {}'.format(os.path.split(outfname)[1],titleid)))
                    resultsq.put(('log','Thread {} started downloading {} for title {}'.format(threadid,os.path.split(outfname)[1],titleid)))
                ...
                with open(outfname, 'wb') as outfile:
                    ...
                    while True:
                         ...
                         if expected_size:
                             if resultsq:
                                 resultsq.put(('progress',len(buf)))
                            
                         outfile.write(buf)
            else:
                resultsq.put(('log', '{} for title {} exists, skipping file.'.format(os.path.split(outfname)[1],titleid)))
                downloaded_size = statinfo.st_size
                resultsq.put(('progress', expected_size))
            infile.close()
            ...
            if expected_size is not None:
                ...
                if dlsize != expected_size:
                    if dlsize < expected_size:
                        resultsq.put(('log','WARNING: {} for title {} finished with a smaller than expected size!'.format(os.path.split(outfname)[1],titleid)))
                    elif dlsize > expected_size:
                        resultsq.put(('log','WARNING: {} for title {} finished with a larger than expected size!'.format(os.path.split(outfname)[1],titleid)))
            ...
            resultsq.put(('log', 'DOWNLOAD ERROR: Could not download {} for title {}'.format(os.path.split(outfname)[1],titleid)))
        else:
            return True
...
def process_title_id(title_id, title_key, name=None, region=None, output_dir=None, retry_count=3, onlinetickets=False, patch_demo=False,
    ...
    # download stuff
    resultq.put(('log','Downloading TMD for {}'.format(title_id)))
    ...
    if not download_file(baseurl + '/tmd', tmd_path, retry_count):
        resultq.put(('log','DOWNLOAD ERROR: failed to download TMD for {}'.format(title_id)))
        return
    ...
    if typecheck == '000e':
        if not download_file(baseurl + '/cetk', os.path.join(rawdir, 'title.tik'), retry_count):
            resultq.put(('log', 'DOWNLOAD ERROR: Could not download ticket from {}, skipping title.'.format(baseurl + '/cetk')))
            return
    elif onlinetickets:
        keysite = get_keysite()
        tikurl = '{}/ticket/{}.tik'.format(keysite, title_id)
        if not download_file(tikurl, os.path.join(rawdir, 'title.tik'), retry_count):
            resultq.put(('log', 'DOWNLOAD ERROR: Could not download ticket from {}, skipping title.'.format(keysite)))
            return
    else:
        make_ticket(title_id, title_key, title_version, os.path.join(rawdir, 'title.tik'), patch_demo, patch_dlc)

    if tickets_only:
        resultq.put(('log', 'Ticket, TMD, and CERT completed for {}. Not downloading contents.'.format(title_id)))
        return
